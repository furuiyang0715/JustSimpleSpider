...
def create_table_gov_stats():
    ...
    # 国家统计局 - 数据解读
    sql = """
        DROP TABLE IF EXISTS `gov_stats_sjjd`;

        CREATE TABLE `gov_stats_sjjd` (
          `id` int(11) NOT NULL AUTO_INCREMENT,
          `pub_date` datetime not null  COMMENT '发布时间',
          `title` varchar(64) collate utf8_bin DEFAULT NULL COMMENT '文章标题',
          `link` varchar(128) collate utf8_bin DEFAULT NULL COMMENT '文章详情页链接',
          ...
          PRIMARY KEY (`id`), 
          KEY `pub_date` (`pub_date`)
        ) ENGINE=InnoDB AUTO_INCREMENT=38 DEFAULT CHARSET=utf8mb4 COMMENT='国家统计局[数据解读]'; 

        ALTER TABLE `gov_stats_sjjd` ADD unique(`link`); 

        show full columns from gov_stats_sjjd;
        """

    # 国家统计局--统计动态
    sql = """
    DROP TABLE IF EXISTS `gov_stats_tjdt`;

    CREATE TABLE `gov_stats_tjdt` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
      `pub_date` datetime not null  COMMENT '发布时间',
      `title` varchar(64) collate utf8_bin DEFAULT NULL COMMENT '文章标题',
      `link` varchar(128) collate utf8_bin DEFAULT NULL COMMENT '文章详情页链接',
      ...
      PRIMARY KEY (`id`), 
      KEY `pub_date` (`pub_date`)
    ) ENGINE=InnoDB AUTO_INCREMENT=38 DEFAULT CHARSET=utf8mb4 COMMENT='国家统计局[统计动态]'; 

    ALTER TABLE `gov_stats_tjdt` ADD unique(`link`); 

    show full columns from gov_stats_tjdt;
    """
